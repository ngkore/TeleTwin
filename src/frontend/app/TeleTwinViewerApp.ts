...
export class TeleTwinViewerApp {
  ...
  public static ipcCall = new Proxy({} as IpcMethods, {
    get(_target, key: keyof IpcMethods): AsyncFunction {
      ...
      switch (key) {
        case "getConfig":
          return async () => {
            if (!TeleTwinViewerApp._config) {
              TeleTwinViewerApp._config = await makeIpcCall("getConfig")([]);
            }
            return TeleTwinViewerApp._config;
          };
        default:
  ...
  public static async getFile(): Promise<string | undefined> {
    const options: OpenDialogOptions = {
      title: TeleTwinViewerApp.translate("open"),
      properties: ["openFile"],
      filters: [{ name: "iModels", extensions: ["ibim", "bim"] }],
    };
    const val = await TeleTwinViewerApp.ipcCall.openFile(options);
  ...
  public static async getFolder(title?: string): Promise<string | undefined> {
    const val = await TeleTwinViewerApp.ipcCall.openFolder({
      title: title || TeleTwinViewerApp.translate("selectFolder")
    });
  ...
  public static async saveBriefcase(
    ...
    const options: SaveDialogOptions = {
      title: TeleTwinViewerApp.translate("saveBriefcase"),
      defaultPath: `${this._getFileName(iModelName)}.bim`,
      filters: [{ name: "iModels", extensions: ["ibim", "bim"] }],
    };
    const val = await TeleTwinViewerApp.ipcCall.saveFile(options);
