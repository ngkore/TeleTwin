...
export const SettingsContextProvider = ({
  ...
  const getRecentSettings = useCallback(async () => {
    const updatedSettings = await TeleTwinViewerApp.ipcCall.getSettings();
  ...
  const addRecent = useCallback(
    async (
      ...
      await TeleTwinViewerApp.ipcCall.addRecentFile({
        TeleTwinId,
        iModelId,
  ...
  const checkFileExists = useCallback(
    async (file: ViewerFile) => {
      const exists = await TeleTwinViewerApp.ipcCall.checkFileExists(file);
      if (!exists) {
        await TeleTwinViewerApp.ipcCall.removeRecentFile(file);
        await getRecentSettings();
      }
      return exists;
    },
