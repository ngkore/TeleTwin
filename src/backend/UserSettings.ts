...
class UserSettings {
    ...
    if (this.settings.recents) {
      return this.settings.recents.findIndex(
        (existingFile) =>
          ...
          (file.iModelId &&
            existingFile.TeleTwinId === file.TeleTwinId &&
  ...
  public get dataPath() {
    if (!this._dataPath) {
      this._dataPath = ElectronHost.app
        .getPath("userData")
        .replace("/Electron", "/TeleTwin Viewer");
  ...
  public addRecent(file: ViewerFile) {
    if (!this.settings.recents) {
      ...
      // use the existing file if one exists as it likely has the TeleTwin and iModel ids, whereas opening a local file would not
      if (existing !== 0) {
